...
pred %>%
  ...
  group_by(YEAR, guild) %>%
  summarise(CPUE_KGKM2 = mean(CPUE_KGKM2), .groups = "drop") -> pred_density
...
status_trends <- paste0("Pacific cod density decreased slightly from 2024 to 2025 in Bristol Bay, and the 2025 
                        estimate is just below the 42-year mean.")
